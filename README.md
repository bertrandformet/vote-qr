...
Outil de vote par QR code pour formateurs. Les participants scannent un QR code avec leur téléphone, leur vote s'enregistre instantanément dans Google Sheets, et les résultats s'affichent en temps réel sur le tableau de bord du formateur.
...
4. Cliquer **▶ Lancer le vote**. Le compteur démarre, les résultats se rafraîchissent toutes les 2 secondes.
...
8. Cliquer **↺ Remettre à zéro** pour effacer les résultats avant un prochain vote.
...
> **Important :** `vote.html` et `dashboard.html` peuvent utiliser **deux déploiements différents** du même Apps Script. À chaque modification du script, créer un nouveau déploiement et mettre à jour l'URL correspondante. Les deux URLs n'ont pas besoin d'être identiques.

**Dans `vote.html`**, ligne `WEBHOOK_ECRITURE` (utilisée pour enregistrer les votes) :

```javascript
const WEBHOOK_ECRITURE = 'https://script.google.com/macros/s/VOTRE_ID_ECRITURE/exec';
```

**Dans `dashboard.html`**, deux constantes à mettre à jour :

```javascript
// Ligne WEBHOOK — utilisée pour reset et remise à zéro
const WEBHOOK = 'https://script.google.com/macros/s/VOTRE_ID_DASHBOARD/exec';
...
Remplacer `VOTRE_ID_ECRITURE` et `VOTRE_ID_DASHBOARD` par les URLs de déploiement Apps Script, et `VOTRE_SHEET_ID` par l'ID du classeur Google Sheets.
...
Téléphone participant
  └─ scanne QR → charge vote.html?c=1..4 (GitHub Pages)
                  └─ fetch GET /exec?action=write&vote=N (Apps Script, fire-and-forget)
                              └─ incrémente cellule A2..D2 (Google Sheets)

Tableau de bord formateur (fichier local)
  └─ pollResults() toutes les 2 s → lit CSV directement depuis Google Sheets (gviz/tq)
  └─ lancerVote()  → GET /exec?action=reset&question=... (Apps Script)
  └─ resetVote()   → GET /exec?action=reset&question=   (Apps Script, remise à zéro)
```
...
La requête vers Apps Script est cross-origin. Avec `no-cors`, le navigateur envoie la requête mais ne lit pas la réponse. C'est suffisant : on n'a besoin que de déclencher l'écriture, pas de lire le résultat. Le fetch est lancé en « fire-and-forget » (sans `await`) pour afficher la confirmation instantanément, sans attendre le cold start d'Apps Script (~4 s).
